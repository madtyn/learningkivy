...
from comics.comicwidgets import StickMan, DraggableWidget
...
class ToolButton(ToggleButton):
    def on_touch_down(self, touch):
        ...
        if self.state == 'down' and ds.collide_point(touch.x, touch.y):
            # Translate the touch coordinates relative to the drawingscreen
...
class ToolStickman(ToolButton):
    def draw(self, ds, x, y):
        ...
        # This makes the stickman to appear at the touch point, and
        # not at the coord origin
        sm.center = (x, y)
        ds.add_widget(sm)
